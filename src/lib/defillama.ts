...
/**
 * Strict Solstice relevance filter.
 * A pool passes ONLY if:
 *   1. At least one token in the symbol is a Solstice token, OR
 *   2. poolMeta contains "solstice" (Kamino Solstice Market), OR
 *   3. Symbol is a PT/YT of a Solstice token (e.g. PT-USX-16SEP26)
 */
function isSolsticeRelevant(pool: DefiLlamaPool): boolean {
  ...
  // Rule 1: symbol contains a Solstice token
  if (tokens.some((t) => SOLSTICE_TOKENS.has(t))) return true;

  // Rule 2: Kamino "Solstice Market" metadata
  if (meta.includes('solstice')) return true;

  // Rule 3: PT/YT of a Solstice token (e.g. PT-USX-16SEP26, PT-EUSX-16SEP26)
